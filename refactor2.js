...
// Replace `return (` right after the `if (nivelPermiso === 'ninguno') { ... }` check
code = code.replace(
  `  return (\n    <div className="flex-1`,
  `${functionDef}\n    <div className="flex-1`
);

// ONLY replace the original occurrence in the right panel section!
// We can locate it precisely by looking for the section tag before it
const rightPanelStart = code.indexOf('<section className="hidden xl:flex flex-col w-80 shrink-0 border-l border-slate-200 bg-slate-50 lg:h-full overflow-y-auto">');
const rightPanelEnd = code.indexOf('</section>', rightPanelStart);

const oldRightPanelContent = code.substring(rightPanelStart, rightPanelEnd + '</section>'.length);
const newRightPanelContent = oldRightPanelContent.replace(
  innerContext,
  '{contexto ? renderContextContent() : null}'
);
code = code.replace(oldRightPanelContent, newRightPanelContent);
