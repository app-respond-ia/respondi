...
import { crearCasoDesdeConversacion } from '@/app/actions/casos'

export default function AgenteChatsPage() {
  ...
  const handleTogglePausa = async (conv: any) => {
    ...
    if (res.success && res.data) {
      setConversaciones(prev => prev.map(c => c.id === conv.id ? { ...c, ...res.data, cases: c.cases } : c))
    }
  }
  ...
  const handleCrearCaso = async (conv: any) => {
    setCreandoCaso(true)
    const res = await crearCasoDesdeConversacion(conv.id)
    if (res.success && res.data) {
      const nuevoCaso = res.data
      setConversaciones(prev => prev.map(c => 
        c.id === conv.id 
          ? { ...c, ia_pausada: true, cases: [...(c.cases || []), nuevoCaso] } 
          : c
      ))
      router.push(`/agente/casos/${nuevoCaso.id}`)
    } else {
      setMensajeGlobal({ tipo: 'error', texto: res.error || 'Error al crear caso' })
    }
  ...
  const getCasoActivo = (conv: any) => {
    if (!conv?.cases) return null
    return conv.cases.find((c: any) => c.estatus === 'pendiente' || c.estatus === 'atendiendo') || conv.cases[0]
  }

  return (
    <div className="h-[calc(100vh-theme(spacing.20))] flex lg:flex-row overflow-hidden bg-white border border-slate-200 rounded-2xl shadow-sm">
      {/* PANEL IZQUIERDO */}
      <section className={`flex flex-col w-full lg:w-96 lg:shrink-0 border-r border-slate-200 bg-white h-full ${selectedConvId ? 'hidden lg:flex' : 'flex'}`}>
        <div className="px-4 h-16 flex items-center gap-3 border-b border-slate-200 shrink-0">
          <h1 className="font-display font-700 text-xl text-ink-900">Chats de sucursal</h1>
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-slate-100">
          {loadingChats ? (
            ...
            filteredConversaciones.map(conv => {
              ...
              const bgAvatar = 'bg-slate-200 text-slate-700'
              const isSelected = selectedConvId === conv.id
              const isPausada = conv.ia_pausada
              const casoAsociado = getCasoActivo(conv)
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-1 flex gap-1 flex-wrap">
                      {isPausada && conv.estado === 'activa' && (
                        <span className="inline-block text-[10px] font-600 bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded">IA pausada</span>
                      )}
                      {casoAsociado && (
                        <span className="inline-block text-[10px] font-600 bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded">Caso asignado</span>
                      )}
                    </div>
                  </div>
                </button>
      ...
      <section className={`flex flex-col flex-1 min-w-0 bg-slate-50 h-full ${!selectedConvId ? 'hidden lg:flex items-center justify-center' : 'flex'}`}>
        {!selectedConvId ? (
          ...
          <>
            {/* Cabecera */}
            <div className="px-4 h-16 flex items-center gap-3 border-b border-slate-200 bg-white shrink-0">
              ...
              <div className="flex items-center gap-3 shrink-0">
                {/* Botones de gestión de caso */}
                {(() => {
                  const casoActivo = getCasoActivo(selectedConv);
                  if (casoActivo) {
                    return (
                      <Link 
                        href={`/agente/casos/${casoActivo.id}`}
                        className="hidden sm:inline-flex px-3 py-1.5 bg-emerald-100 text-emerald-700 hover:bg-emerald-200 rounded-lg text-xs font-600 transition"
                      >
                        Ver caso
                      </Link>
                    )
                    ...
                    return (
                      <button 
                        onClick={() => handleCrearCaso(selectedConv)}
                        disabled={creandoCaso}
                        className="hidden sm:inline-flex px-3 py-1.5 bg-brand-100 text-brand-700 hover:bg-brand-200 disabled:opacity-50 rounded-lg text-xs font-600 transition"
                      >
                        {creandoCaso ? 'Creando...' : 'Crear caso y asignarme'}
                      </button>
                    )
                  }
                })()}
