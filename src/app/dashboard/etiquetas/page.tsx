...
import { COLORES_ETIQUETA, clasesEtiqueta } from '@/lib/etiquetas/colores'

const COLORS = COLORES_ETIQUETA
const getPillClasses = clasesEtiqueta
...
function EtiquetaRow({ item, onToggle, onEdit, onDelete, dragHandleProps, disableDrag, soloLectura }: {
  item: any,
    ...
    <div className="p-4 sm:p-5 flex items-center gap-4 bg-white">
      ...
      {item.es_protegida ? (
        <div className="flex items-center gap-1 shrink-0 border-l border-slate-100 pl-2 ml-1">
          <span title="Etiqueta del sistema — no se puede editar ni eliminar" className="shrink-0 inline-flex items-center justify-center w-5 h-5 rounded-full bg-brand-100/50 text-brand-600">
  ...
  const cargar = async () => {
    ...
    if (permisosRes.success) {
      if ((permisosRes as any).esAdmin) {
        ...
        const p = (permisosRes.data || []).find((p: any) => p.seccion === 'etiquetas')
