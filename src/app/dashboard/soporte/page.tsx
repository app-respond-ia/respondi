...
export default function DashboardSoportePage() {
  ...
  const cargarTickets = async () => {
    setLoading(true)
    const res = await getTicketsCliente()
  ...
  return (
    <div className="space-y-6">
              ...
              <Link href={`/dashboard/soporte/${ticket.id}`} key={ticket.id} className={`flex flex-col sm:flex-row gap-3 sm:gap-6 p-4 sm:p-5 hover:bg-slate-50 transition group ${ticket.fijado ? 'bg-amber-50/30' : ''}`}>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                    ...
                    {ticket.categoria && (
                      <span className="text-[11px] font-600 px-2 py-0.5 rounded-md border border-slate-200 text-slate-600 truncate flex items-center gap-1.5">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-ink-900/40 backdrop-blur-sm">
            ...
            <form onSubmit={handleCrearTicket} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input 
                  ...
                  placeholder="Ej: Problema con la integración de WhatsApp"
