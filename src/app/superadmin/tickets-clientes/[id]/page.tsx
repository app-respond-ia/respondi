...
import { 
  getTicketDetalleClienteSuperadmin, 
  responderTicketCliente, 
  asignarCategoriaPrioridadCliente, 
  cambiarEstatusTicketCliente, 
  getCategoriasTicketsClientes, 
  getSuperadmins, 
  asignarTicketCliente 
...
export default function TicketDetalleClienteSuperadminPage() {
  const { id } = useParams()
  ...
  const cargarDetalle = useCallback(async () => {
    const res = await getTicketDetalleClienteSuperadmin(id as string)
    if (res.success && res.data) {
      setTicket(res.data)
    } else {
      router.push('/superadmin/tickets-clientes')
      return
    }
  ...
  useEffect(() => {
    ...
    const setupRealtime = async () => {
      ...
      channel = supabase
        .channel(`client-ticket-admin-${id}`)
        .on(
          'postgres_changes',
  ...
  const handleEnviarMensaje = async (e: React.FormEvent) => {
    ...
    const res = await responderTicketCliente(id as string, nuevoMensaje)
  ...
  const handleChangeCategoria = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    await asignarCategoriaPrioridadCliente(ticket.id, newVal, ticket.prioridad)
  }

  const handleChangePrioridad = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    const newVal = e.target.value
    setTicket({ ...ticket, prioridad: newVal })
    await asignarCategoriaPrioridadCliente(ticket.id, ticket.categoria_id, newVal)
  }

  const handleAsignar = async (userId: string | null) => {
    setTicket({ ...ticket, asignado_a: userId })
    await asignarTicketCliente(ticket.id, userId)
  }
  ...
  const handleToggleEstatus = async () => {
    ...
    const res = await cambiarEstatusTicketCliente(ticket.id, newStatus)
    if (res.success) {
    ...
    <div className="max-w-5xl mx-auto flex flex-col lg:flex-row gap-6 h-[calc(100vh-8rem)]">
      ...
      <div className="flex-1 flex flex-col space-y-4">
        ...
        <div className="shrink-0">
          <Link href="/superadmin/tickets-clientes" className="inline-flex items-center gap-1.5 text-sm font-600 text-brand-600 hover:text-brand-700 transition mb-4">
          ...
          <p className="text-sm text-ink-500 mt-1">
            Cliente: <span className="font-600 text-ink-700">{ticket.creador?.nombre}</span> 
            {ticket.organizacion?.nombre && ` (${ticket.organizacion.nombre})`}
          </p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-6 custom-scrollbar bg-white rounded-2xl border border-slate-200 p-4 sm:p-6 shadow-sm">
          {(!ticket.mensajes || ticket.mensajes.length === 0) ? (
            ...
            ticket.mensajes.map((msg: any) => {
              const esCliente = msg.user_id === ticket.user_id || msg.user?.rol !== 'super_admin'
              
              return (
                <div key={msg.id} className={`flex gap-4 ${esCliente ? '' : 'flex-row-reverse'}`}>
                  <div className={`w-10 h-10 rounded-full shrink-0 flex items-center justify-center font-700 text-sm uppercase ${esCliente ? 'bg-slate-200 text-slate-600' : 'bg-brand-100 text-brand-700'}`}>
                    {msg.user?.nombre ? msg.user.nombre.substring(0, 2) : (esCliente ? 'C' : 'S')}
                  </div>
                  <div className={`max-w-[80%] border rounded-2xl p-4 shadow-sm ${esCliente ? 'bg-white border-slate-200 rounded-tl-none' : 'bg-brand-50 border-brand-100 rounded-tr-none'}`}>
                    <div className="flex justify-between items-start gap-4 mb-2">
                      <p className="text-sm font-700 text-ink-900">
                        {msg.user?.nombre || (esCliente ? 'Cliente' : 'Soporte')}
                      </p>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-5 shadow-sm sticky top-0">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              {canWrite && <Link href="/superadmin/tickets-clientes/categorias" className="text-xs font-500 text-brand-600 hover:text-brand-700 transition">Gestionar categorías</Link>}
            </div>
            <select 
          ...
          {canWrite && (
            <div className="pt-2">
              ...
              >
                {isTogglingEstatus ? 'Actualizando...' : (ticket.estatus === 'cerrado' ? 'Reabrir ticket' : 'Marcar como resuelto')}
              </button>
            </div>
          )}
          ...
          {ticket.calificacion && (
            <div className="border-t border-slate-100 pt-4 mt-2">
              <h4 className="font-600 text-sm text-ink-900 mb-2">Valoración del Cliente</h4>
