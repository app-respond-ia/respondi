...
import { 
  ...
  crearCategoriaTicketCliente, 
  actualizarCategoriaTicketCliente, 
  borrarCategoriaTicketCliente 
...
export default function CategoriasTicketsClientesPage() {
  ...
  const cargarCategorias = async () => {
    setLoading(true)
    const res = await getCategoriasTicketsClientes()
  ...
  const handleConfirmarBorrar = async () => {
    ...
    setSaving(true)
    const res = await borrarCategoriaTicketCliente(confirmarBorrar)
    setSaving(false)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let res
    if (modalMode === 'añadir') {
      res = await crearCategoriaTicketCliente(nombre, color)
    } else {
      res = await actualizarCategoriaTicketCliente(editingId!, nombre, color)
    }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex justify-between items-center bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
        <div>
          <Link href="/superadmin/tickets-clientes" className="inline-flex items-center gap-1.5 text-sm font-600 text-brand-600 hover:text-brand-700 transition mb-2">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7"/></svg>
            Volver a tickets de clientes
          </Link>
