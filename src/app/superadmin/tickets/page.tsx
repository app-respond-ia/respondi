              ...
              <tbody className="divide-y divide-slate-100">
                {filtrados.map(t => (
                  <tr key={t.id} className={`hover:bg-slate-50 transition ${t.fijado ? 'bg-amber-50/30' : ''}`}>
                    ...
                    <td className="px-5 py-3.5 text-ink-600 whitespace-nowrap">{t.vendedores?.nombre || '—'}</td>
                    <td className="px-5 py-3.5">
                      <div className="flex flex-wrap gap-1">
                        {t.ticket_categorias ? (
                          <span className="px-1.5 py-0.5 rounded text-[10px] font-600 border" style={{ backgroundColor: `${t.ticket_categorias.color}15`, color: t.ticket_categorias.color, borderColor: `${t.ticket_categorias.color}30` }}>
