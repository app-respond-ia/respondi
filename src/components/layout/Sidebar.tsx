  ...
  return (
    <aside className={`relative ${collapsed ? 'w-20' : 'w-72'} bg-ink-900 text-white flex flex-col h-full transition-all duration-300 ease-out`}>
      ...
      <nav className="flex-1 px-3 py-5 space-y-1 overflow-y-auto scrollbar-hide">
        ...
        {(() => {
          const nivel = getNivel('agenda')
          const cls = navItemWithPermClass('/dashboard/agenda', 'agenda')
          return nivel === 'ninguno' ? null : (
            <Link href="/dashboard/agenda" onClick={onCloseMobile} className={cls} title={collapsed ? 'Agenda' : undefined}>
              <span className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center shrink-0">
                <svg className="w-4.5 h-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M8 2v3M16 2v3M3.5 9h17M5 4h14a1.5 1.5 0 0 1 1.5 1.5v14A1.5 1.5 0 0 1 19 21H5a1.5 1.5 0 0 1-1.5-1.5v-14A1.5 1.5 0 0 1 5 4z"/><path strokeLinecap="round" strokeLinejoin="round" d="M8 13h3M8 17h5"/></svg>
              </span>
        ...
        {(() => {
          const nivel = getNivel('canales')
          const cls = navItemWithPermClass('/dashboard/tienda', 'canales')
          return nivel === 'ninguno' ? null : (
            <Link href="/dashboard/tienda" onClick={onCloseMobile} className={cls} title={collapsed ? 'Tienda online' : undefined}>
              <span className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center shrink-0">
                <svg className="w-4.5 h-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/></svg>
              </span>
              {!collapsed && <span className="truncate">Tienda online</span>}
            </Link>
          )
        })()}
        {(() => {
          const nivel = getNivel('canales')
          const cls = navItemWithPermClass('/dashboard/automatizaciones', 'canales')
          return nivel === 'ninguno' ? null : (
            <Link href="/dashboard/automatizaciones" onClick={onCloseMobile} className={cls} title={collapsed ? 'Automatizaciones' : undefined}>
              <span className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center shrink-0">
                <svg className="w-4.5 h-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
              </span>
              {!collapsed && <span className="truncate">Automatizaciones</span>}
            </Link>
          )
        })()}
        </>
