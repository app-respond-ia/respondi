...
export default function SuperadminNotificationBell({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    ...
    const setupRealtime = async () => {
      ...
      channel = supabase
        .channel('notificaciones-superadmin')
        .on(
          'postgres_changes',
          {
            event: 'INSERT',
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute top-0 right-0 inline-flex items-center justify-center w-4 h-4 text-[10px] font-bold leading-none text-white bg-rose-500 rounded-full border-2 border-white transform translate-x-1 -translate-y-1">
