...
// piden a la vez.
export async function conEnlacesDeArchivos<T extends { media_url?: string | null }>(mensajes: T[] | null | undefined): Promise<(T & { media_enlace?: string | null })[]> {
  const lista = mensajes || []
  const rutas = [...new Set(lista.map(m => rutaDe(m.media_url)).filter(Boolean) as string[])]
  ...
  return lista.map(m => {
    ...
    return ruta ? { ...m, media_enlace: porRuta.get(ruta) || null } : m
  })
}
